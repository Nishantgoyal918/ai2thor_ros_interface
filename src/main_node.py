...
config = {
    ...
    # Ai2Thor Params
    'AGENT_MODE': "locobot",
    'VISIBILITY_DIST': 1.5, #100, #1.5,

    'SCENE': "FloorPlan210", #"FloorPlan_Train1_3",
...
class Ai2ThorNode:
    ...
    def __init__(
        ...
        self.tf_forward_x = 0.0
        self.tf_forward_y = 0.0
        self.tf_theta_ = 0.0 #-90.0
        ################################################
    ...
    def publish_tf(
        ...
        # forward_ = forward_ + self.tf_forward_
        # theta_ = theta_ + self.tf_theta_

        theta_ = theta_ + self.tf_theta_
        new_x = forward_ * math.cos(theta_ * math.pi / 180.0)
        new_y = forward_ * math.sin(theta_ * math.pi / 180.0)

        new_y = self.tf_forward_y + new_y
        new_x = self.tf_forward_x + new_x

        self.tf_broadcaster.sendTransform((new_x, new_y, 0.0),
                                        tf.transformations.quaternion_from_euler(0, 0, theta_ * math.pi / 180.0),
                                        rospy.Time.now(),
                                        "base_link",
                                        "odom")

        self.tf_forward_x = new_x
        self.tf_forward_y = new_y
        self.tf_theta_ = theta_

        # self.tf_forward_ = forward_
        # self.tf_theta_ = theta_
    ...
    def step(
        ...
        if action_msg is None:
            ...
            tf_forward_ = action_msg['FORWARD']
        ...
        #########################################NEW CODE
        self.publish_tf(forward_=tf_forward_, theta_=tf_theta_)
        #################################################
